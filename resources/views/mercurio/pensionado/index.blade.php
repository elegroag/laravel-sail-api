...
<link rel="stylesheet" href="{{ versioned_asset('mercurio/build/Mercurio38.css') }}" />
@endpush
...
<script type="text/template" id='tmp_table'>
    @include('mercurio/pensionado/tmp/tmp_table')
    </script>

<script type="text/template" id='tmp_create'>
    @include('mercurio/pensionado/tmp/tmp_create')
    </script>
...
<script>
    const _TITULO = "{{ $title }}";
    window.ServerController = 'pensionado';
    window.BREADCRUMB_TITLE = "{{ $title }}";
</script>

<script src="{{ versioned_asset('mercurio/build/Mercurio38.js') }}"></script>
@endpush