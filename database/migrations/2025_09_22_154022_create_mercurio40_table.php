...
return new class extends Migration
{
    ...
    {
        Schema::create('mercurio40', function (Blueprint $table) {
            // Motor InnoDB
            $table->engine = 'InnoDB';
            ...
            $table->index('ruuid', 'fk_mercurio40_ruuid');
            $table->unique('ruuid', 'unique_mercurio40_ruuid');

            // FKs con los nombres del SQL
            $table->foreign(['tipo', 'coddoc', 'documento'], 'fk_mercurio31_mercurio0710000')
                ->references(['tipo', 'coddoc', 'documento'])
                ->on('mercurio07')
                ->onDelete('cascade')
                ->onUpdate('cascade');

            $table->foreign('codest', 'fk_mercurio31_mercurio1110000')
                ->references('codest')
                ->on('mercurio11')
                ->onDelete('cascade')
                ->onUpdate('cascade');
    ...
    {
        Schema::table('mercurio40', function (Blueprint $table) {
            $table->dropForeign('fk_mercurio31_mercurio0710000');
            $table->dropForeign('fk_mercurio31_mercurio1110000');
            $table->dropIndex('fk_mercurio31_mercurio071_idx');
            $table->dropIndex('fk_mercurio31_mercurio111_idx');
            $table->dropIndex('log');
            $table->dropUnique('id');
            $table->dropIndex('fk_mercurio40_ruuid');
            $table->dropUnique('unique_mercurio40_ruuid');
        });
        Schema::dropIfExists('mercurio40');
    }
};
