...
{
    private $orderpag = 'ORDER BY fecini DESC';
    private $tipopc = 2;
    ...
    {
        $entity = new Mercurio30();
        return $entity->find($query, $this->orderpag);
    }
    ...
    public function findByUserAndEstado(Srequest $request)
    {
        ...
        foreach ($data as $row) {
            ...
            if ($row->getEstado() == 'P') {
                if ($dias_vencidos == 3) $style = '#d3a246';
                if ($dias_vencidos > 3) $style = '#ff6161';
            } else {
                $style = '#61b5ff';
            }
