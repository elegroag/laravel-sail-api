...
class RetiroService
{
    ...
    public function findAllByEstado($estado = '')
    {
        ...
        if (empty($estado)) {
            $conditions = "and m35.estado NOT IN('I') ";
        } else {
            $conditions = "and m35.estado='{$estado}' ";
        }

        $sql = "SELECT m35.*,
            (SELECT COUNT(*) FROM mercurio10 as me10 WHERE me10.tipopc='13' and m35.id = me10.numero) as cantidad_eventos,
            (SELECT MAX(fecsis) FROM mercurio10 as mr10 WHERE mr10.tipopc='13' and m35.id = mr10.numero) as fecha_ultima_solicitud,
            (CASE
                WHEN m35.estado = 'T' THEN 'Temporal en edición'
                WHEN m35.estado = 'D' THEN 'Devuelto'
                WHEN m35.estado = 'A' THEN 'Aprobado'
                WHEN m35.estado = 'X' THEN 'Rechazado'
                WHEN m35.estado = 'P' THEN 'Pendiente De Validación CAJA'
                WHEN m35.estado = 'I' THEN 'Inactiva'
            END) as estado_detalle,
            coddoc as tipo_documento
            FROM mercurio35 as m35
    ...
    {
        $trabajador = new Mercurio35($data);
        $trabajador->save();
        $id = $trabajador->getId();
    ...
    {
        return Mercurio35::where('id', $id)->first();
    }
    ...
    public function consultaSeguimiento($id)
    {
        ...
        return [
            ...
            'campos_disponibles' => (new Mercurio35)->CamposDisponibles(),
    ...
    {
        $datos = Mercurio35::select('cedtra as cedula', DB::raw("CONCAT_WS('', prinom, segnom, priape, segape) as nombre_completo"))
            ->where([
                ['documento', $documento],
                ['coddoc', $coddoc],
                ['estado', '<>', 'X'],
