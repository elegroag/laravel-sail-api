...
import FacultativosView from './views/FacultativosView';

export default class FacultativoListas extends ControllerValidation {
	tipo = null;
	headerMain = null;
	headerList = null;
	...
	listRequests(tipo = '', pagina = 0) {
		...
		const view = new FacultativosView({
			collection: {
				tipo: this.tipo,
				pagina: pagina,
			},
	...
	loadHeaders() {
		this.headerMain = new HeaderCajasView({
			model: {
				...
				detalle: 'Aprobar facultativos ' + this.__estados[this.tipo],
				info: true,
			},
		});
