...
class ServicioDomesticoModel extends Backbone.Model {
	constructor(options) {
		super(options);
		this.on('change:tipdoc', (model) => {
			let coddocrepleg = ServicioDomesticoModel.changeTipdoc(model.get('tipdoc'));
	...
	validate(attr = {}) {
		return RulesValidator(ServicioDomesticoModel.Rules.rules, attr);
	}
	...
	static Rules = {
		...
		messages: {
			...
			fecsol: { required: 'Se requiere del campo fecha' },
	...
	static changeRulesProperty(transfer = []) {
		_.each(transfer, (row) => ServicioDomesticoModel.changeRuleProperty(row));
	}
}

export { ServicioDomesticoModel };
