...
class GeneralService
{
    ...
    public function sendEmail2($correo, $nombre, $asunto, $msj, $file = '')
    {
        ...
        $mcontenido .= "<img style='display:block;border:none' src='" . $rutaImg . "' width='30%' height='' title='Sistemas Y Solucuiones Integradas' alt='Caja de Compensación Familiar del Caquetá'>";
        $mcontenido .= '</td>';
        $mcontenido .= '</tr>';
        $mcontenido .= '<tr>';
        ...
        $mcontenido .= '<tbody>';
        $mcontenido .= '<tr>';
        $mcontenido .= "<td height='50' valign='middle' align='left' style='font-family:Helvetica,Arial;font-size:11px;color:#8e8e8e'>Mercurio - Caja de Compensación Familiar del CaquetáS.A.S - 2019</td>";
        $mcontenido .= '</tbody>';
        $mcontenido .= '</table>';
        $mcontenido .= '</div>';
        $mcontenido .= '</td>';
    ...
    public function consultaTrabajador($mercurio31)
    {
        ...
        if ($datos_captura['flag'] == true) {
            ...
            $_tipcon = [];
            foreach ($datos_captura['tipcon'] as $data) {
                $_tipcon[$data['tipcon']] = $data['detalle'];
            }
            $_trasin = [];
            foreach ($datos_captura['trasin'] as $data) {
                $_trasin[$data['trasin']] = $data['detalle'];
            }
            $_vivienda = [];
            foreach ($datos_captura['vivienda'] as $data) {
                $_vivienda[$data['vivienda']] = $data['detalle'];
    ...
    public function consultaConyuge($mercurio32)
    {
        ...
        if ($datos_captura['flag'] == true) {
            ...
            $_comper = [];
            foreach ($datos_captura['comper'] as $data) {
                $_comper[$data['comper']] = $data['detalle'];
            }
            $_codciu = [];
            foreach ($datos_captura['codciu'] as $data) {
                $_codciu[$data['codciu']] = $data['detalle'];
            ...
            $_codocu = [];
            foreach ($datos_captura['codocu'] as $data) {
                $_codocu[$data['codocu']] = $data['detalle'];
            }
            $_captra = [];
            foreach ($datos_captura['captra'] as $data) {
                $_captra[$data['captra']] = $data['detalle'];
    ...
    public function consultaBeneficiario($mercurio34)
    {
        ...
        if ($datos_captura['flag'] == true) {
            ...
            $_parent = [];
            foreach ($datos_captura['parent'] as $data) {
                $_parent[$data['parent']] = $data['detalle'];
            }
            $_huerfano = [];
            foreach ($datos_captura['huerfano'] as $data) {
                $_huerfano[$data['huerfano']] = $data['detalle'];
            }
            $_tiphij = [];
            foreach ($datos_captura['tiphij'] as $data) {
                $_tiphij[$data['tiphij']] = $data['detalle'];
            }
            $_nivedu = [];
            foreach ($datos_captura['nivedu'] as $data) {
                $_nivedu[$data['nivedu']] = $data['detalle'];
            ...
            $_calendario = [];
            foreach ($datos_captura['calendario'] as $data) {
                $_calendario[$data['calendario']] = $data['detalle'];
            }
        }
    ...
    public function startTrans($models) {}
