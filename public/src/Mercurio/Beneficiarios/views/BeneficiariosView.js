...
class BeneficiariosView extends Backbone.View {
	...
	render() {
		...
		this.trigger('load:table', {
			url: this.model.tipo
				? 'beneficiario/renderTable/' + this.model.tipo
				: 'beneficiario/renderTable',
			callback: (html) => {
	...
	cambioCuenta(event) {
		...
		Swal.fire({
			...
			if (result.value) {
				const __controller = $("[name='csrf-token']").attr('controller');
				let _url = $App.kumbiaURL(__controller + '/' + target.attr('data-href'));
