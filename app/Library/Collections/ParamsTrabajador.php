...
class ParamsTrabajador
{
    ...
    static $orientacion_sexual;
    static $labora_otra_empresa;
    ...
    {
        foreach (self::$datos_captura['orientacion_sexuales'] as $data) self::$orientacion_sexual[$data['codori']] = $data['detalle'];
        return self::$orientacion_sexual;
    }

    public static function getVulnerabilidades()
    {
        foreach (self::$datos_captura['vulnerabilidades'] as $data) self::$vulnerabilidades[$data['codigo']] = $data['nombre'];
    ...
    {
        foreach (self::$datos_captura['rural'] as $data) self::$rural[$data['rural']] = $data['detalle'];
        return self::$rural;
    }

    public static function getTipoContrato()
    {
        foreach (self::$datos_captura['tipcon'] as $data) self::$tipo_contrato[$data['tipcon']] = $data['detalle'];
        return self::$tipo_contrato;
    }

    public static function getSindicalizado()
    {
        foreach (self::$datos_captura['sidicalizado'] as $data) self::$sidicalizado[$data['trasin']] = $data['detalle'];
        return self::$sidicalizado;
    }

    public static function getVivienda()
    {
        foreach (self::$datos_captura['vivienda'] as $data) self::$vivienda[$data['vivienda']] = $data['detalle'];
    ...
    public static function getTipoAfiliado()
    {
        foreach (self::$datos_captura['tipo_cotizantes'] as $data) {
            if (is_null($data['circular'])) continue;
    ...
    {
        foreach (self::$datos_captura['cabhog'] as $data) self::$cabeza_hogar[$data['cabhog']] = $data['detalle'];
    ...
    {
        foreach (self::$datos_captura['ciudades'] as $data) self::$ciudades[$data['codciu']] = $data['codciu'] . ' - ' . $data['detciu'];
        return self::$ciudades;
    }

    public static function getTiposDocumentos()
    {
        foreach (self::$datos_captura['tipo_documentos'] as $data) self::$tipos_documentos[$data['coddoc']] = $data['codrua'];
        return self::$tipos_documentos;
    }

    public static function getOcupaciones()
    {
        foreach (self::$datos_captura['ocupaciones'] as $data) self::$ocupaciones["{$data['codocu']}"] = $data['codocu'] . ' ' . $data['detalle'];
        return self::$ocupaciones;
    }

    public static function getLaboraOtraEmpresa()
    {
        foreach (self::$datos_captura['labora_otra_empresa'] as $data) self::$labora_otra_empresa[$data['estado']] = $data['detalle'];
