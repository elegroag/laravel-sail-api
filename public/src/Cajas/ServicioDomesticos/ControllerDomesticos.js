...
import { DomesticoInfoView } from './DomesticoInfoView';
import { $App } from '@/App';
import { DomesticosView } from './DomesticosView';

class ControllerDomesticos extends ControllerValidation {
	constructor(options = {}) {
		super(options);
	}
		...
		$App.trigger('syncro', {
			...
			callback: (response) => {
				if (response) {
					...
					const view = new DomesticoInfoView({
						model: entity,
						collection: {
							solicitud: solicitud,
