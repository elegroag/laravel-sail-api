    ...
    public function consulta_indicadoresAction(Request $request)
    {
        ...
        foreach ($gener02 as $mgener02) {
            ...
            foreach ($mercurio09 as $mmercurio09) {
                ...
                $html .= "<tr>";
                if ($first) $html .= "<th class='align-middle' rowspan='" . $mercurio09->count() . "'>{$mgener02->getUsuario()}-{$mgener02->getNombre()}</th>";
                $first = false;
                $html .= "<td>{$mmercurio09->getDetalle()}</td>";
                ...
                foreach ($mercurio as $mmercurio) {
                    ...
                    if ($dias_vencidos > $mmercurio09->getDias() && $mmercurio->getEstado() == 'P') $total_vencido++;
                }
