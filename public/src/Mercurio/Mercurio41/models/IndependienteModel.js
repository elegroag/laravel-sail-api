...
class IndependienteModel extends Backbone.Model {
    constructor(options = {}) {
        super(options);
        this.on('change:tipdoc', (model) => {
            let coddocrepleg = IndependienteModel.changeTipdoc(model.get('tipdoc'));
    ...
    static Rules = {
        rules: {
            ...
            cedtra: { required: true, minlength: 5 },
            ...
            fecsol: { required: false, date: true },
    ...
    static changeRulesProperty(transfer = []) {
        _.each(transfer, (row) => IndependienteModel.changeRuleProperty(row));
    }
}

export { IndependienteModel };
