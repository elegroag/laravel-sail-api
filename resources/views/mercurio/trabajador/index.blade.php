...
@push('scripts')
    ...
    <script type="text/template" id='tmp_table'>
        @include('mercurio/trabajador/tmp/tmp_table')
    </script>

    <script type="text/template" id='tmp_create'>
        @include('mercurio/trabajador/tmp/tmp_create')
    </script>
    ...
    <script>
        const _TITULO = "{{ $title }}";
        window.ServerController = 'trabajador';
    </script>

    <script src="{{ asset('mercurio/build/Trabajadores.js') }}"></script>
@endpush