		...
		this.subHeader = new SubHeaderView({
			...
			collection: [
				...
				},
				{
					...
					hidden:
						this.model.get('id') &&
						(this.model.get('estado') === 'D' || this.model.get('estado') === 'T')
							? false
							: true,
			...
			callback: (response) => {
				if (response.success) {
					...
					this.listenTo(
						this.viewDocuments,
						'file:trash',
						this.gestionAdjuntoService.borrarArchivo,
					);
					this.listenTo(
						this.viewDocuments,
						'file:save',
						this.gestionAdjuntoService.guardarArchivo,
					);
					this.listenTo(
						this.viewDocuments,
						'file:prodoc',
						this.gestionAdjuntoService.processDocument,
					);
