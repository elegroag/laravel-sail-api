...
import FacultativoInfoView from './views/FacultativoInfoView';
import FacultativoAprobarModel from './models/FacultativoAprobarModel';

export default class FacultativoInformation extends ControllerValidation {
	tipo = null;
	headerMain = null;
	headerView = null;
	solicitudModel = null;
	...
	infoRequest(collection = {}) {
		...
		const view = new FacultativoInfoView({
			model: new FacultativoAprobarModel({ id: this.solicitudModel.get('id') }),
	...
	loadHeaders() {
		this.headerMain = new HeaderCajasView({
			model: {
				titulo: 'Aprobar facultativo',
				detalle: 'Facultativo con identificación ' + this.solicitudModel.get('cedtra'),
				info: false,
			},
		});
