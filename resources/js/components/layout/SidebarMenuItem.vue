<template>
  <div>
    ...
    <button
      ...
      :class="[
        'flex w-full items-center gap-2.5 rounded-md px-2.5 py-2 text-[12px] transition-all duration-150',
        'hover:bg-white/10',
        ...
        <li v-for="child in item.children" :key="child.id">
          ...
          <div v-if="child.children && child.children.length > 0">
            <button
              ...
              :class="[
                'flex w-full items-center gap-2 rounded-md px-2.5 py-1.5 text-[11.5px] transition-colors',
              ...
              <ul v-if="expandedChildren.has(child.id)" class="mt-0.5 space-y-0.5 pl-4">
                <li v-for="grandchild in child.children" :key="grandchild.id">
                  <Link
                    ...
                    custom
                    v-slot="{ navigate, isActive: gcActive }"
                  >
                    <a
                      @click="() => { navigate(); $emit('navigate', grandchild.route || '') }"
                      :class="[
          ...
          >
            <a
              @click="() => { navigate(); $emit('navigate', child.route || '') }"
              :class="[
