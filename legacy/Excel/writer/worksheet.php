...
*/
    function writeString($row, $col, $str, $format = null)
    {
        ...
        // ✅ PHP 8 safe: garantizar string
        $str = (string) $str;

        $stren  = strlen($str);
        $record = 0x0204;                 // Record identifier
        $length = 0x0008 + $stren;        // Bytes to follow
        $xf     = $this->_XF($format);    // The cell format
        ...
        if ($stren > $this->_xls_strmax) { // LABEL must be < 255 chars
            $str       = substr($str, 0, $this->_xls_strmax);
            $length    = 0x0008 + $this->_xls_strmax;
            $stren    = $this->_xls_strmax;
            $str_error = -3;
        }

        $header    = pack("vv",   $record, $length);
        $data      = pack("vvvv", $row, $col, $xf, $stren);
    ...
    {
        $bitmap_array = (object)$this->_processBitmap($bitmap);
