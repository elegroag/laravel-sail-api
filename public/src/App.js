...
const $App = {
	...
	alert(type, transfer) {
		const {
			title = 'Notificación',
			message = 'Nota no hay respuesta de la solicitud',
			timer = 8200,
		} = transfer;
	...
	url(method = '', controller = undefined) {
		const __controller = $Kumbia.controller;
		const route = controller ?? __controller;
		return this.kumbiaURL(route + '/' + method);
	},
	kumbiaURL: (url) => {
		if (_.isUndefined(url) === true) url = '';
		const path = $Kumbia.path;
		const app = $Kumbia.app;
		if (app != '') {
			return path + app + '/' + url;
		} else {
			return path + url;
