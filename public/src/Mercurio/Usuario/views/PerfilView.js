...
class PerfilView extends Backbone.View {
    ...
    editarDatos(e) {
        e.preventDefault();
        this.remove();
        $App.router.navigate('editar', { trigger: true });
    }
    ...
    closeEdit(e) {
        ...
        $App.trigger('confirma', {
            ...
            callback: (status) => {
                if (status) {
                    this.remove();
                    $App.router.navigate('datos', { trigger: true });
                }
            },
        });
    }
    ...
    changeClaveNew(e) {
        ...
        if (valor.length > 0) {
            ...
            if (clave1 !== valor) {
                this.$el.find('#show_error_clave').text('Error la clave no es igual a la anterior');
                            ...
                            if (response) {
                                if (response.success) {
                                    ...
                                    Swal.fire({
                                        ...
                                        window.location.href = $App.kumbiaURL('mercurio/login/salir');
                                    });
                                } else {
