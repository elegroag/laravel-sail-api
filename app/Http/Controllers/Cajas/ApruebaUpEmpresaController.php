...
use App\Services\CajaServices\DevolucionNotificacion;
...
class ApruebaUpEmpresaController extends ApplicationController
{
    ...
    public function infoActualiza($id = 0)
    {
        ...
        $this->setParamToView('mercurio11', Mercurio11::get());
    ...
    public function loadParametrosView()
    {
        ...
        $_tipdur = [];
        foreach ($datos_captura['tipo_duracion'] as $data) {
            $_tipdur[$data['estado']] = $data['detalle'];
        }
        $_codind = [];
        foreach ($datos_captura['codigo_indice'] as $data) {
            $_codind[$data['codind']] = $data['detalle'];
        }
        $_todmes = [];
        foreach ($datos_captura['paga_mes'] as $data) {
            $_todmes[$data['estado']] = $data['detalle'];
        }
        $_forpre = [];
        foreach ($datos_captura['forma_presentacion'] as $data) {
            $_forpre[$data['estado']] = $data['detalle'];
        }
        $_pymes = [];
        foreach ($datos_captura['pymes'] as $data) {
            $_pymes[$data['estado']] = $data['detalle'];
        }
        $_contratista = [];
        foreach ($datos_captura['contratista'] as $data) {
            $_contratista[$data['estado']] = $data['detalle'];
        }
        $_tipemp = [];
        ...
        $_tipapo = [];
        foreach ($datos_captura['tipo_aportante'] as $data) {
            $_tipapo[$data['estado']] = $data['detalle'];
        }
        $_ofiafi = [];
        foreach ($datos_captura['oficina'] as $data) {
            $_ofiafi["{$data['ofiafi']}"] = $data['detalle'];
        }
        $_colegio = [];
        foreach ($datos_captura['colegio'] as $data) {
            $_colegio["{$data['estado']}"] = $data['detalle'];
        }
        $_tipper = [];
        ...
        foreach ($datos_captura['ciudad_comercial'] as $data) {
            $_ciupri["{$data['codciu']}"] = $data['detciu'];
        }
        $_codzon = [];
        ...
        $_coddocrepleg = [];
        foreach ($datos_captura['tipo_documentos'] as $data) {
            if ($data['codrua'] == 'TI' || $data['codrua'] == 'RC') {
                continue;
            }
