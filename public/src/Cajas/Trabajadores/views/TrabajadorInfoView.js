        ...
        switch (this.$el.find('#tippag').val()) {
            ...
            case 'D':
                ...
                if (is_numeric(_numcue)) {
                    this.$el.find('#numcue-error').text('Debe tener formato de número de cuenta correcto.');
                ...
                if (response) {
                    if (response.multi == true) {
                        $App.trigger('confirma', {
                            message: 'El trabajador esta afiliado con otra empresa, desea registrar la actual ?',
                            callback: (status) => {
                                if (status) {
