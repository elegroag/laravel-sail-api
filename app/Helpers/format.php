...
if (!function_exists('dump')) {
    ...
    function dump($var, $is_str = false)
    {
        $rtn = preg_replace(
            array('/Array\s+\(/', '/\[(\d+)\] => (.*)\n/', '/\[([^\d].*)\] => (.*)\n/'),
            array('array (', '\1 => \'\2\'' . "\n", '\'\1\' => \'\2\'' . "\n"),
...
if (!function_exists('dd')) {
    function dd($msj, $key = '', $sanetizar = true)
    {
        $isCli = (php_sapi_name() == 'cli');
        if ($isCli) {
            if (is_array($key)) {
                echo "\033[91m" . "\nLa key no puede ser tipo array\n" . "\033[0m";
                return false;
            }
            if ($key != '') {
                if (is_array($msj)) {
                    echo "\033[93m" . "\nData Type Array" . "\033[0m";
                }
                if (is_object($msj)) {
                    echo "\033[93m" . "\nnData Type Object" . "\033[0m";
                }
                if (is_string($msj)) {
                    echo "\033[93m" . "\nnData Type String" . "\033[0m";
                }
                if (is_numeric($msj)) {
                    echo "\033[93m" . "\nnData Type Numeric" . "\033[0m";
                }
            }
            if (is_array($msj) || is_object($msj)) {
                ob_start();
                var_export($msj);
                $msj = ob_get_contents();
                ob_end_clean();
            ...
            if ($sanetizar) {
                $msj = str_replace(array('stdClass', '::', '__set_statearray', 'array', '(', ')', '\'', "\"", ">"), array(''), $msj);
            }
            $msj = str_replace(',', "\r", $msj);
            echo "\033[92m" . "\ndd--------------------\n" . "\033[0m";
            echo "\033[97m" . "{$key} -> {$msj}" . "\033[40m";
            echo "\033[92m" . "\nend--------------------\n" . "\033[0m";
        } else {

            if (is_array($key)) {
                echo '<pre style="background-color: #e0f7fa; border: 1px solid #b2ebf2; padding: 10px; margin: 10px 0; border-radius: 4px; overflow-x: auto; font-family: monospace; font-size: 14px; line-height: 1.5;">';
                echo "La key no puede ser tipo array";
                echo '</pre>';
                return false;
            }
            ...
            echo '<span style="color: #07d400ff;">dump--------------------</span><br>'; // Cian
            echo '<code style="color: #333;">';
            var_dump($msj);
            echo '</code>';
            echo '<span style="color: #07d400ff;"><br>end--------------------</span>';
            echo '</pre>';
        }
        die();
    }
