...
export default class RegisterNotyView extends ModelView {
    ...
    sendNotify(e) {
        ...
        if (telefono == '') {
            this.$el.find('#telefono-error').text('El teléfono o celular de contacto es un valor requerido.');
        ...
        if (archivo.length > 0) {
            if (!this._handleFile('archivo')) {
                this.$el.find('#archivo-error').text('El archivo no posee un formato valido, solo se admiten: imagenes, word o pdf');
        ...
        this.#App.trigger('confirma', {
            ...
            callback: (status) => {
                if (status) {
                    const url = this.#App.url('procesar_notificacion', window.ServerController ?? 'notificaciones');
