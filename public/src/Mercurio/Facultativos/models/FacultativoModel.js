...
class FacultativoModel extends Backbone.Model {
	constructor(options) {
		super(options);
		this.on('change:tipdoc', (model) => {
			let coddocrepleg = FacultativoModel.changeTipdoc(model.get('tipdoc'));
	...
	static changeRulesProperty(transfer = []) {
		_.each(transfer, (row) => FacultativoModel.changeRuleProperty(row));
	}
}

export { FacultativoModel };
