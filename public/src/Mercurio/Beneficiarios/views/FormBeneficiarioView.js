...
export class FormBeneficiarioView extends FormView {
	...
	#afterRender($el = {}) {
		...
		this.selectores = this.$el.find(
			'#tipdoc, #ciunac, #codban, #biocodciu, #resguardo_id, #pub_indigena_id, #peretn',
		);
		...
		if (this.model.get('id') !== null) {
			...
			if (this.model.get('parent') == '1')
				this.$el.find('#show_mother').removeClass('d-none');
			this.__hasBiologico(
				this.model.get('parent') == '1' || this.model.get('parent') == '4',
			);

			this.$el
				.find("[name='biodesco'][value='N']")
				.attr('checked', this.model.get('biodesco') != 'S');
			this.$el
				.find("[name='biodesco'][value='S']")
				.attr('checked', this.model.get('biodesco') == 'S');

			this.$el
				.find("[name='biourbana'][value='N']")
				.attr('checked', this.model.get('biourbana') == 'N');
			this.$el
				.find("[name='biourbana'][value='S']")
				.attr('checked', this.model.get('biourbana') != 'N');

			if (this.model.get('parent') == '4') this.__changeTipHijo(true);
			if (this.model.get('parent') == '5') this.__changeCustodia(true);
			if (this.model.get('parent') == '2' || this.model.get('parent') == '3')
				this.__changePadres(true);
	...
	saveFormData(event) {
		...
		if (entity.isValid() === false) {
			...
			$App.trigger('alert:warning', {
				message:
					'Alerta algunos de los campos son requeridos: <br/> ' +
		...
		$App.trigger('confirma', {
			message:
				'Confirma que desea guardar los datos del formulario y continuar el proceso de solicitud de afiliación',
			callback: (status) => {
				if (status) {
